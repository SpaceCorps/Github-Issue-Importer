//! Non-secret account metadata in a readable YAML file. The GitHub token itself never lands here -
...
pub struct AccountConfig {
    ...
    pub user: String,
    /// Human name or display name from GitHub profile
    pub name: String,
    /// Email associated with the account / profile
    pub email: String,
    pub added_at: String,
}
...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("GH_ISSUE_IMPORT_CONFIG_DIR")
        .or_else(|| std::env::var_os("GITHUB_ISSUE_IMPORTER_CONFIG_DIR"))
        .filter(|v| !v.is_empty())
    {
    ...
    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("github-issue-importer");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("github-issue-importer")
        } else {
            std::env::var_os("XDG_CONFIG_HOME")
                .filter(|v| !v.is_empty())
                .map(PathBuf::from)
                .unwrap_or_else(|| home.join(".config"))
                .join("github-issue-importer")
        }
    };
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: gh-issue-import accounts add <name>", path.display()))
    })
}
