//! Where GitHub tokens live: the OS keystore, one entry per account under `account:{name}`.
//!
//! DPAPI on Windows, the Keychain on macOS, libsecret on Linux. When none is available the tool
//! refuses to start rather than silently writing a file - `GH_ISSUE_IMPORT_ALLOW_PLAINTEXT_STORE=1` is
...
const SERVICE: &str = "github-issue-importer";
...
fn build() -> Result<Store> {
    if let Some(forced) = std::env::var("GH_ISSUE_IMPORT_SECRET_STORE")
        .or_else(|_| std::env::var("GITHUB_ISSUE_IMPORTER_SECRET_STORE"))
        .ok()
        .filter(|s| !s.trim().is_empty())
    {
        return match forced.to_lowercase().as_str() {
            ...
            _ => Err(Error::invalid(format!("Secret store '{forced}' is not a backend available on this platform."))
                .fix("Unset the secret store env var, or set it to one of: dpapi, keychain, libsecret, plaintext.")),
        };
    }
    ...
    fallback(
        "libsecret is not installed, so there is no OS keystore to hold your tokens. \
         Install it with: sudo apt install libsecret-tools  (or the equivalent for your distro).",
    )
}

fn fallback(reason: &str) -> Result<Store> {
    if std::env::var("GH_ISSUE_IMPORT_ALLOW_PLAINTEXT_STORE").as_deref() == Ok("1")
        || std::env::var("GITHUB_ISSUE_IMPORTER_ALLOW_PLAINTEXT_STORE").as_deref() == Ok("1")
    {
        return Ok(Store::Plaintext);
    }
    Err(Error::other("No secure credential store is available on this machine.").detail(reason).fix(
        "Install a keystore, or set GH_ISSUE_IMPORT_ALLOW_PLAINTEXT_STORE=1 to store tokens in a 0600 file instead.",
    ))
}
...
fn warn_plaintext() {
    ...
    WARNED.get_or_init(|| {
        eprintln!(
            "warning: Tokens are stored unencrypted in {} (GH_ISSUE_IMPORT_ALLOW_PLAINTEXT_STORE=1).",
...
mod dpapi {
    ...
    pub fn protect(plain: &[u8]) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(
                Error::other("DPAPI could not encrypt the tokens.").detail(std::io::Error::last_os_error().to_string())
            );
        }
        Ok(take(out))
    }
    ...
    pub fn unprotect(data: &[u8], path: &Path) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("The stored tokens could not be decrypted.")
                .detail(format!(
                    "DPAPI failed: {}. This happens when the file was written by a different Windows user or on a different machine.",
                    std::io::Error::last_os_error()
                ))
                .fix(format!("Delete {} and run: gh-issue-import accounts add <name>", path.display())));
        }
        Ok(take(out))
    }
}
